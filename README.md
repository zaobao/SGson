...
超出16位范围的Unicode字符，尤其是表情符在现今已经非常普及。这些字符在主流高级编程语言中由UTF16表示为2个char，这2个char，一个在高位代理字符范围内，一个在低位代理字符范围内。SGson在序列化字符串时，若遇到代理字符，不会去校验是否合乎Unicode规范，仅仅是将char通过TextReader写入流中；反序列化字符串时，也仅仅是将char放到UTF16编码的C#字符串中，即使遇到单个的高（低）位代理字符，也不会报错。总之，SGson仅仅是将对象序列化到C#的string类型，具体在写入流时从string到编码字节的转换SGon无法控制。

一般无须担心，一般高级语言对文本流的编码都是一致的。

如果传输或读写的对等方是用Rust语言的，需要注意，因为Rust中的char是32位的，所以Rust的String中并不使用UTF-16的高位代理和低位代理。因此，你必须确保写入流的TextReader的实现把高位代理和低位代理的两个char当做一个字符处理，而不是分别编码。

例如"😃",UTF-16LE编码是4字节3D D8 03 DE，UTF-8编码也是4字节F0 9F 98 83。如果将UTF16的两个char分别用UTF8编码的话将会产生3 * 2 = 6个字节，这样，在使用普通高级语言反序列化时可以被当做两个UTF16的char而兼容，但在Rust中可能会被分别识别为两个UTF-8编码的字符（Rust字符串在内存中的编码是UTF-8），而不是这一个32位表情符（用UTF-8编码也是4字节32位）本身。

所幸，.Net至少从4.0版本开始的自带的TextReader都是使用标准UTF-8编码，把超出16位的Unicode字符，即两个UTF-16编码的char，当做一个字符处理，写入流的是4个字节。
...
如果在JSON中是数组，会被反序列化为List<object>类型，这里的泛型object类型也使用此规则反序列化；  
