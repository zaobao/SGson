...
仿照Google Gson写一个C#下的JSON库，但API会依照C#的一些特性有所调整，自定义DateTime的序列化/反序列化方式时，请使用注册委托的方式。
...
如果我想修改内建的日期格式，比如XML标准的"yyyy-MM-ddTHH-mm-ss"，请使用注册委托的方式来修改，下面章节中有示例。
...
需要注意：内建的数字类型、字符串和DateTime的解析都是通过注册委托的方式定义，所以这两种格式若要自定义，使用TypeAdapter是无效的，推荐使用注册委托的方式方式。
