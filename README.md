...
如果传输或读写的对等方是用Rust语言的，需要注意，因为Rust中的char是32位的，所以Rust的String中并不存在高位代理和低位代理。因此，你必须确保写入流的TextReader的实现把高位代理和低位代理的两个char当做一个字符处理，而不是分别编码。

例如"😃"，UTF-16LE编码是4字节3D D8 03 DE，UTF-8编码也是4字节F0 9F 98 83。如果将UTF16的两个char分别用UTF8编码的话将会产生3 * 2 = 6个字节，这样，在使用普通高级语言反序列化时可以被当做两个UTF16的char而兼容，但在Rust中可能会被分别识别为两个32位的字符，而不是这一个32位表情符本身。
