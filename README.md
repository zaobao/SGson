...
#### 数字类型无损转换
在EMCASript标准中，只有一种数字类型，以IEEE754规定的双精度浮点数为准，也就是说不计符号位，有效二进制位数为53，但SGson并不以double作为通用的数字类型：long的精度保持在63位；ulong的精度保持在64位；并且decimal类型，也不会被强转成double而导致精度损失。

使用SGon，只要序列化和反序列化两边数字类型一致，就可以无损拷贝。
